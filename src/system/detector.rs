...
impl Yolov8Config {
    pub async fn get_bounding_box(&self, img: &DynamicImage) -> anyhow::Result<Vec<DetectionBox>> {
        let new_yolo = Yolov8::new(self)?;
        let acc = self.acc_threshold;
        let nms = self.nms_threshold;
        let img = img.clone();
        log::info!("Model running");
        let ret = spawn_blocking(move || new_yolo.yolo.report_boxes(&img, acc, nms)).await;
...
impl Yolov8 {
    pub fn new(config: &Yolov8Config) -> anyhow::Result<Self> {
    ...
    pub fn open(path: &Path, size: ModelSize) -> anyhow::Result<Self> {
